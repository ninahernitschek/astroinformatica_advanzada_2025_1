...
def MedianBRP(mag):
    ...
    """
        ...
        median = np.median(def)
        amplitude = (np.max(def) - np.min(def)) / 10
        n = len(def)

        count = np.sum(np.logical_and(def < median + amplitude,
                                      def > median - amplitude))
...
def FluxPercentileRatioMid65(Base):
...
def FluxPercentileRatioMid80(Base):
